...
import { publishBatch } from "./publish-review.ts";
...
async function main() {
  ...
  let skipped = 0;
  const rendered: MemeRecord[] = [];
  for (const rec of records) {
    try {
      await renderMeme(rec, getTemplate(rec.template), join(outDir, rec.file));
      rendered.push(rec);
  ...
  const published = await publishBatch(rendered, outDir);
  console.log(`[make] done: ${rendered.length} rendered, ${skipped} skipped; ${published} published to /memes review -> ${manifestPath}`);
}
